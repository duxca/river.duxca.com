...
export HOST_ADDR="${HOST_ADDR:-${SERVER_HOST}:${SERVER_PORT}}"
...
export GCS_BUCKET_NAME="${GCS_BUCKET_NAME:-duxca-litestream-sandbox}"
export LOCAL_DIST_PATH="${LOCAL_DIST_PATH:-target/site}"
export LEPTOS_OUTPUT_NAME="${LEPTOS_OUTPUT_NAME:-leptos-browser}"
export LEPTOS_SITE_ROOT="${LEPTOS_SITE_ROOT:-${LOCAL_DIST_PATH}}"
export LEPTOS_SITE_PKG_DIR="${LEPTOS_SITE_PKG_DIR:-pkg}"
export LEPTOS_SITE_ADDR="${LEPTOS_SITE_ADDR:-${HOST_ADDR}}"
export LEPTOS_RELOAD_PORT="${LEPTOS_RELOAD_PORT:-18082}"

if ! cargo leptos --help >/dev/null 2>&1; then
  echo "cargo-leptos is required. Install with: cargo install --locked cargo-leptos" >&2
  exit 1
fi
